...
| network           | string                  | "default"                              | VPC name.<br>When using "PRIVATE_SERVICE_ACCESS" or "PRIVATE_SERVICE_CONNECT" connect-mode, network needs to be the full VPC name. |
| reserved-ipv4-cidr| string		              | ""                                     | CIDR range to allocate Filestore IP Ranges from.<br>The CIDR must be large enough to accommodate multiple Filestore IP Ranges of /29 each, /26 if enterprise tier is used. Cannot be used with "PRIVATE_SERVICE_CONNECT". |
| reserved-ip-range | string		              | ""                                     | IP range to allocate Filestore IP Ranges from.<br>This flag is used instead of "reserved-ipv4-cidr" when "connect-mode" is set to "PRIVATE_SERVICE_ACCESS" and the value must be an [allocated IP address range](https://cloud.google.com/compute/docs/ip-addresses/reserve-static-internal-ip-address).<br>The IP range must be large enough to accommodate multiple Filestore IP Ranges of /29 each, /26 if enterprise tier is used. Cannot be used with "PRIVATE_SERVICE_CONNECT". |
| connect-mode      | "DIRECT_PEERING"<br>"PRIVATE_SERVICE_ACCESS"<br>"PRIVATE_SERVICE_CONNECT" | "DIRECT_PEERING"  | The network connect mode of the Filestore instance.<br>To provision Filestore instance with shared-vpc from service project, "PRIVATE_SERVICE_ACCESS" or "PRIVATE_SERVICE_CONNECT" mode must be used. "PRIVATE_SERVICE_CONNECT" requires an existing service connection policy and the driver currently only supports IPv4 endpoints. The PSC endpoint is created in the VPC host project. See the [Filestore PSC documentation](https://cloud.google.com/filestore/docs/configure-psc). |
