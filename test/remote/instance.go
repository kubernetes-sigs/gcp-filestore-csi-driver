...
func (i *InstanceInfo) CreateOrGetInstance(serviceAccount string) error {
	...
	err = wait.Poll(15*time.Second, 5*time.Minute, func() (bool, error) {
		...
		klog.Infof("Instance %v in state RUNNING and available by SSH", i.name)
		return true, nil
	})
...
func (i *InstanceInfo) createDefaultFirewallRule() error {
	...
	if _, err = i.computeService.Firewalls.Get(i.project, defaultFirewallRule).Do(); err != nil {
		klog.Infof("Default firewall rule %v does not exist, creating", defaultFirewallRule)
		f := &compute.Firewall{
		...
		klog.Infof("Default firewall rule %v already exists, skipping creation", defaultFirewallRule)
	}
	return nil
}
