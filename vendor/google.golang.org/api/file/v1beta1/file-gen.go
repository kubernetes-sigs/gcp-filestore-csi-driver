...
func NewProjectsLocationsService(s *Service) *ProjectsLocationsService {
	...
	rs.VmAttachments = NewProjectsLocationsVmAttachmentsService(s)
	return rs
}
...
type ProjectsLocationsService struct {
	...
	VmAttachments *ProjectsLocationsVmAttachmentsService
}
...
func NewProjectsLocationsSharePoolsService(s *Service) *ProjectsLocationsSharePoolsService {
	rs := &ProjectsLocationsSharePoolsService{s: s}
	return rs
}
...
func NewProjectsLocationsVmAttachmentsService(s *Service) *ProjectsLocationsVmAttachmentsService {
	rs := &ProjectsLocationsVmAttachmentsService{s: s}
	return rs
}

type ProjectsLocationsVmAttachmentsService struct {
	s *Service
}

// AcquireShareRequest: Request message for AcquireShare.
type AcquireShareRequest struct {
	// CapacityGb: Required. Requested size of the share in GiB.
	CapacityGb int64 `json:"capacityGb,omitempty,string"`
	// RequestId: Required. Unique string generated by the CSI driver for exact
	// retry idempotency.
...
func (s AcquireShareRequest) MarshalJSON() ([]byte, error) {
	type NoMethod AcquireShareRequest
	return gensupport.MarshalJSON(NoMethod(s), s.ForceSendFields, s.NullFields)
}

// AcquireShareResponse: Output message for AcquireShare.
type AcquireShareResponse struct {
	// IpAddress: The IP address of the physical Filestore instance hosting the
	// share.
	IpAddress string `json:"ipAddress,omitempty"`
	// ShareId: The specific share identifier on the instance.
...
func (s AcquireShareResponse) MarshalJSON() ([]byte, error) {
	type NoMethod AcquireShareResponse
	return gensupport.MarshalJSON(NoMethod(s), s.ForceSendFields, s.NullFields)
}

// Attachment: Attachment contains all the metadata associated with a Filestore
// instance VM attachment.
type Attachment struct {
	// CapacityGb: Output only. The capacity of the attached Filestore instance in
	// gigabytes (GB). Filestore defines 1 GB as 1024^3 bytes.
	CapacityGb int64 `json:"capacityGb,omitempty,string"`
	// Instance: Required. The unique identifier of the attached Filestore
	// instance. Only VM attachable instances can be used here. Attempting to use a
	// non VM attachable instance will result in this Attachment entering a FAILURE
	// state. Format:
	// `projects/{project}/locations/{location}/instances/{instance}`.
	Instance string `json:"instance,omitempty"`
	// MountMode: Optional. The mount mode of this attachment.
	//
	// Possible values:
	//   "MOUNT_MODE_UNSPECIFIED" - Mount mode not specified.
	//   "READ_WRITE" - Mount permissions of a Filestore instance are read and
	// write.
	//   "READ_ONLY" - Mount permissions of a Filestore instance are read-only.
	MountMode string `json:"mountMode,omitempty"`
	// MountPoint: Required. The mount point on which filestore_id is mounted on.
	// Maximum length of a mount point is limited to 200 chars. On non-Windows VMs,
	// the mount path will be created if it does not exist, if it already exist, it
	// will be used (the mount path would not be deleted when the Attachment is
	// detached). Format: Non-Windows VM mount point must start with '/'. Windows
	// VM mount point must be a drive letter from the set ["A:", ..., "Z:"],
	// excluding "C:".
	MountPoint string `json:"mountPoint,omitempty"`
	// Network: Output only. The name of VPC network to which the Filestore
	// instance is connected to, e.g., "default", "my-network". For more
	// information - see https://cloud.google.com/vpc/docs/vpc
	Network string `json:"network,omitempty"`
	// State: Output only. The state of the attachment.
	//
	// Possible values:
	//   "STATE_UNKNOWN" - State is unknown.
	//   "ATTACHING" - The attachment is pending to be attached.
	//   "ACTIVE" - The attachment is ready.
	//   "FAILURE" - The attachment is in a failure state.
	State string `json:"state,omitempty"`
	// StateDetails: Output only. Details regarding the state of the attachment
	// e.g., if there's a problem, what is the problem and what actions may resolve
	// it. This field may contain linked text in markdown format. The content of
	// this field is intended for human consumption and should not be parsed by
	// machines.
	StateDetails string `json:"stateDetails,omitempty"`
	// Tier: Output only. The service tier of the attached Filestore instance.
	//
	// Possible values:
...
type FileShareConfig struct {
	...
	// Enabled: Service enabled. When enabled, the instance exposes the exports to
	// the user for mounting.
	...
	//
	// Possible values:
	//   "FILE_SHARE_PROTOCOL_UNSPECIFIED" - MODE_UNSPECIFIED serves a "not set"
	// default value when a FileShareProtocol is a separate field in a message.
	//   "NFS_V3" - NFS 3.0
	//   "NFS_V4_0" - NFS 4.0
	//   "NFS_V4_1" - NFS 4.1
	//   "SMB_V2_0" - SMB 2.0
...
// GoogleApiServicecontrolV1Operation: Represents information regarding an
// operation.
type GoogleApiServicecontrolV1Operation struct {
	...
	// api`_`key:API_KEY. spatula_header: and "loas:" are also accepted.
	...
	// the consumer project. **IMPORTANT**: DO NOT USE. Although never implemented
	// externally, this was implemented for internal consumers/clients. However,
	// this is being deprecated. Please see: - go/ct:migrate-to-otlp -
	// go/ct:1p-ingestion-integration-policy - go/ct:drz-and-legacy-integ-proposal
...
type Instance struct {
	...
	// EkmKeyUnreachableTime: Output only. The time when the instance noticed the
	// key is unreachable due to EKM.
	EkmKeyUnreachableTime string `json:"ekmKeyUnreachableTime,omitempty"`
	...
	//   "LIMITED" - The instance is limited for use and operations might fail
	// during this state. The reasons are listed in state_reasons.
	//   "PROMOTING" - The replica instance is being promoted.
	State string `json:"state,omitempty"`
	// StateReasons: Output only. Field indicates all the reasons for the instance
	// being in a certain state.
	//
	// Possible values:
	//   "STATE_REASON_UNSPECIFIED" - Reason not specified.
	//   "STATE_REASON_EKM_KEY_UNREACHABLE" - The EKM key used by the instance is
	// unreachable.
	...
	// denied access to.
	//   "EKM_KEY_UNREACHABLE" - The EKM key used by the instance is unreachable.
...
// ListVmAttachmentsResponse: ListVmAttachmentsResponse is the result of
// ListVmAttachmentsRequest.
type ListVmAttachmentsResponse struct {
	// NextPageToken: The standard List next-page token. If not empty, indicates
	// that there are more VM attachments that match the request. Provide this
	// value in a subsequent request to return the next page of results.
	NextPageToken string `json:"nextPageToken,omitempty"`
	// Unreachable: Unordered list. Locations that could not be reached.
	Unreachable []string `json:"unreachable,omitempty"`
	// VmAttachments: A list of VM attachments in the project for the specified
	...
	// unreachable, the response will only return instances in reachable locations
...
func (s ListVmAttachmentsResponse) MarshalJSON() ([]byte, error) {
	type NoMethod ListVmAttachmentsResponse
	return gensupport.MarshalJSON(NoMethod(s), s.ForceSendFields, s.NullFields)
}
...
type NetworkConfig struct {
	...
	// Subnetwork: Output only. The name of the Google Compute Engine subnetwork
	// (https://cloud.google.com/vpc/docs/vpc) to which the instance is connected.
...
// ReleaseShareRequest: Request message for ReleaseShare.
type ReleaseShareRequest struct {
	// IpAddress: Required. The IP address of the physical Filestore instance
	// hosting the share.
	IpAddress string `json:"ipAddress,omitempty"`
	// ShareId: Required. The specific share ID on the instance.
	ShareId string `json:"shareId,omitempty"`
	// ForceSendFields is a list of field names (e.g. "IpAddress") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
...
func (s ReleaseShareRequest) MarshalJSON() ([]byte, error) {
	type NoMethod ReleaseShareRequest
	return gensupport.MarshalJSON(NoMethod(s), s.ForceSendFields, s.NullFields)
}

// ReleaseShareResponse: Response message for ReleaseShare.
...
type ReportRequest struct {
	...
	// ServiceName: The service name as specified in its service configuration. For
	// example, "pubsub.googleapis.com". See google.api.Service
...
type ResourceInfo struct {
	// Permission: The resource permission required for this request. Should be an
	// IAM Permission with the format "{service}.{plural}.{verb}".
	...
	// ResourceName: Name of the resource. This is used for auditing purposes. The
	// value of this field will be used for proto.resource_name field of
...
// between spans in a trace. This message is a subset of
// google.devtools.cloudtrace.v2.Span. Note that this message corresponds to a
// legacy Cloud Trace API. New Cloud Trace ingestion should be done using the
// "Telemetry API", which alignes with the Open Telemetry representation of a
// span. Please do not use in any new code (except for the purposes of enabling
// migration from writing to Chemist to writing to the Telemetry API). See
// also: go/ct:migrate-to-otlp
...
// specified length. This message is equivalent with
...
// VmAttachment: VmAttachment captures all the metadata information Filestore
// service needs to know about a Compute Engine virtual machine (VM) instance
// and its Filestore attachments. A Filestore VmAttachment corresponds to
// exactly one GCE VM instance, and may contain multiple Filestore instance
// attachments for this VM.
type VmAttachment struct {
	// Attachments: The Filestore attachments configured for this VmAttachment. The
	// maximum number of Filestore attachments per VM is 20. Each attachment must
	// have a unique mount point.
	Attachments []*Attachment `json:"attachments,omitempty"`
	// Name: Identifier. The resource identifier of this VM attachment, including
	// the VM's project, location, and name (those are the identifiers of the
	// corresponding Compute Engine instance). Format:
	// `projects/{project}/locations/{location}/vmAttachments/{vm_name}`.
	Name string `json:"name,omitempty"`
	// VmId: Optional. The VM ID of the GCE VM instance. To view the ID of a VM,
	// view the details of the VM and then view the value of the id field. For
	// instructions on how to view the details of a VM see:
...
func (s VmAttachment) MarshalJSON() ([]byte, error) {
	type NoMethod VmAttachment
	return gensupport.MarshalJSON(NoMethod(s), s.ForceSendFields, s.NullFields)
}
...
func (c *ProjectsLocationsInstancesResumeReplicaCall) doRequest(alt string) (*http.Response, error) {
	...
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v1beta1/{+name}:resumeReplica")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("POST", urls, body)
	if err != nil {
		return nil, err
...
type ProjectsLocationsSharePoolsAcquireShareCall struct {
	s                   *Service
	parent              string
	acquiresharerequest *AcquireShareRequest
	urlParams_          gensupport.URLParams
	ctx_                context.Context
	header_             http.Header
}

// AcquireShare: Acquires a share synchronously from the pre-provisioned share
// pool.
//
//   - parent: The parent share pool path, in the format
//     `projects/{project}/locations/{location}/sharePools/{share_pool}`.
func (r *ProjectsLocationsSharePoolsService) AcquireShare(parent string, acquiresharerequest *AcquireShareRequest) *ProjectsLocationsSharePoolsAcquireShareCall {
	c := &ProjectsLocationsSharePoolsAcquireShareCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.parent = parent
	c.acquiresharerequest = acquiresharerequest
	return c
}
...
// https://developers.google.com/gdata/docs/2.0/basics#PartialResponse for more
// details.
func (c *ProjectsLocationsSharePoolsAcquireShareCall) Fields(s ...googleapi.Field) *ProjectsLocationsSharePoolsAcquireShareCall {
	c.urlParams_.Set("fields", googleapi.CombineFields(s))
	return c
}

// Context sets the context to be used in this call's Do method.
func (c *ProjectsLocationsSharePoolsAcquireShareCall) Context(ctx context.Context) *ProjectsLocationsSharePoolsAcquireShareCall {
	c.ctx_ = ctx
	return c
}
...
// Header returns a http.Header that can be modified by the caller to add
// headers to the request.
func (c *ProjectsLocationsSharePoolsAcquireShareCall) Header() http.Header {
	if c.header_ == nil {
		c.header_ = make(http.Header)
	}
	return c.header_
...
func (c *ProjectsLocationsSharePoolsAcquireShareCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "application/json", c.header_)
	body, err := googleapi.WithoutDataWrapper.JSONBuffer(c.acquiresharerequest)
	if err != nil {
		return nil, err
	}
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v1beta1/{+parent}:acquireShare")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("POST", urls, body)
	if err != nil {
		return nil, err
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "file.projects.locations.sharePools.acquireShare", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}

// Do executes the "file.projects.locations.sharePools.acquireShare" call.
// Any non-2xx status code is an error. Response headers are in either
// *AcquireShareResponse.ServerResponse.Header or (if a response was returned
// at all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
func (c *ProjectsLocationsSharePoolsAcquireShareCall) Do(opts ...googleapi.CallOption) (*AcquireShareResponse, error) {
	gensupport.SetOptions(c.urlParams_, opts...)
	res, err := c.doRequest("json")
	if res != nil && res.StatusCode == http.StatusNotModified {
		if res.Body != nil {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "file.projects.locations.sharePools.acquireShare", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}

type ProjectsLocationsSharePoolsReleaseShareCall struct {
	s                   *Service
	parent              string
	releasesharerequest *ReleaseShareRequest
	urlParams_          gensupport.URLParams
	ctx_                context.Context
	header_             http.Header
}

// ReleaseShare: Releases a share synchronously, marking it for background
// scrubbing.
//
//   - parent: The parent share pool path, in the format
//     `projects/{project}/locations/{location}/sharePools/{share_pool}`.
func (r *ProjectsLocationsSharePoolsService) ReleaseShare(parent string, releasesharerequest *ReleaseShareRequest) *ProjectsLocationsSharePoolsReleaseShareCall {
	c := &ProjectsLocationsSharePoolsReleaseShareCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.parent = parent
	c.releasesharerequest = releasesharerequest
	return c
}
...
// https://developers.google.com/gdata/docs/2.0/basics#PartialResponse for more
// details.
func (c *ProjectsLocationsSharePoolsReleaseShareCall) Fields(s ...googleapi.Field) *ProjectsLocationsSharePoolsReleaseShareCall {
	c.urlParams_.Set("fields", googleapi.CombineFields(s))
	return c
}

// Context sets the context to be used in this call's Do method.
func (c *ProjectsLocationsSharePoolsReleaseShareCall) Context(ctx context.Context) *ProjectsLocationsSharePoolsReleaseShareCall {
	c.ctx_ = ctx
	return c
}
...
// Header returns a http.Header that can be modified by the caller to add
// headers to the request.
func (c *ProjectsLocationsSharePoolsReleaseShareCall) Header() http.Header {
	if c.header_ == nil {
		c.header_ = make(http.Header)
	}
	return c.header_
...
func (c *ProjectsLocationsSharePoolsReleaseShareCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "application/json", c.header_)
	body, err := googleapi.WithoutDataWrapper.JSONBuffer(c.releasesharerequest)
	if err != nil {
		return nil, err
	}
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v1beta1/{+parent}:releaseShare")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("POST", urls, body)
	if err != nil {
		return nil, err
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "file.projects.locations.sharePools.releaseShare", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}

// Do executes the "file.projects.locations.sharePools.releaseShare" call.
// Any non-2xx status code is an error. Response headers are in either
// *ReleaseShareResponse.ServerResponse.Header or (if a response was returned
// at all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
func (c *ProjectsLocationsSharePoolsReleaseShareCall) Do(opts ...googleapi.CallOption) (*ReleaseShareResponse, error) {
	gensupport.SetOptions(c.urlParams_, opts...)
	res, err := c.doRequest("json")
	if res != nil && res.StatusCode == http.StatusNotModified {
		if res.Body != nil {
...
type ProjectsLocationsVmAttachmentsGetCall struct {
	s            *Service
	name         string
	urlParams_   gensupport.URLParams
	ifNoneMatch_ string
...
// Get: Gets the details of a specific VM attachment. The resulting
// VmAttachment.attachments represents the configured desired state, which
// might somewhat differ from the current state in the VM's operating system,
// as it might take a while to be applied. Detaching an Attachment is
// immediately reflected in the configured desired state, and detach operations
// are actuated as soon as possible, though they might take a while to execute
// when the VM is active.
//
//   - name: The vm attachment resource name, in the format
//     `projects/{project}/locations/{location}/vmAttachments/{vm_attachment}`.
func (r *ProjectsLocationsVmAttachmentsService) Get(name string) *ProjectsLocationsVmAttachmentsGetCall {
	c := &ProjectsLocationsVmAttachmentsGetCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.name = name
	return c
}
...
// https://developers.google.com/gdata/docs/2.0/basics#PartialResponse for more
// details.
func (c *ProjectsLocationsVmAttachmentsGetCall) Fields(s ...googleapi.Field) *ProjectsLocationsVmAttachmentsGetCall {
	c.urlParams_.Set("fields", googleapi.CombineFields(s))
	return c
}
...
// object's ETag matches the given value. This is useful for getting updates
// only after the object has changed since the last request.
func (c *ProjectsLocationsVmAttachmentsGetCall) IfNoneMatch(entityTag string) *ProjectsLocationsVmAttachmentsGetCall {
	c.ifNoneMatch_ = entityTag
	return c
}

// Context sets the context to be used in this call's Do method.
func (c *ProjectsLocationsVmAttachmentsGetCall) Context(ctx context.Context) *ProjectsLocationsVmAttachmentsGetCall {
	c.ctx_ = ctx
	return c
}
...
// Header returns a http.Header that can be modified by the caller to add
// headers to the request.
func (c *ProjectsLocationsVmAttachmentsGetCall) Header() http.Header {
	if c.header_ == nil {
		c.header_ = make(http.Header)
	}
	return c.header_
...
func (c *ProjectsLocationsVmAttachmentsGetCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "file.projects.locations.vmAttachments.get", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}

// Do executes the "file.projects.locations.vmAttachments.get" call.
// Any non-2xx status code is an error. Response headers are in either
// *VmAttachment.ServerResponse.Header or (if a response was returned at all)
// in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to check
// whether the returned error was because http.StatusNotModified was returned.
func (c *ProjectsLocationsVmAttachmentsGetCall) Do(opts ...googleapi.CallOption) (*VmAttachment, error) {
	gensupport.SetOptions(c.urlParams_, opts...)
	res, err := c.doRequest("json")
	if res != nil && res.StatusCode == http.StatusNotModified {
		if res.Body != nil {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "file.projects.locations.vmAttachments.get", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}

type ProjectsLocationsVmAttachmentsListCall struct {
	s            *Service
	parent       string
	urlParams_   gensupport.URLParams
	ifNoneMatch_ string
...
// List: Lists all VM attachments in a project for either a specified location
// or for all locations.
//
//   - parent: The project and location for which to retrieve instance
//     information, in the format `projects/{project}/locations/{location}`. In
//     Cloud Filestore, locations map to Google Cloud zones, for example
//     **us-west1-b**. To retrieve VM attachments information for all locations,
//     use "-" for the `{location}` value.
func (r *ProjectsLocationsVmAttachmentsService) List(parent string) *ProjectsLocationsVmAttachmentsListCall {
	c := &ProjectsLocationsVmAttachmentsListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.parent = parent
	return c
}

// Filter sets the optional parameter "filter": List filter. See
// https://google.aip.dev/160 for more details.
func (c *ProjectsLocationsVmAttachmentsListCall) Filter(filter string) *ProjectsLocationsVmAttachmentsListCall {
	c.urlParams_.Set("filter", filter)
	return c
}
...
// OrderBy sets the optional parameter "orderBy": Sort results. Supported
// values are "name", "name desc" or "" (unsorted).
func (c *ProjectsLocationsVmAttachmentsListCall) OrderBy(orderBy string) *ProjectsLocationsVmAttachmentsListCall {
	c.urlParams_.Set("orderBy", orderBy)
	return c
}
...
func (c *ProjectsLocationsVmAttachmentsListCall) PageSize(pageSize int64) *ProjectsLocationsVmAttachmentsListCall {
	c.urlParams_.Set("pageSize", fmt.Sprint(pageSize))
	return c
}
...
// PageToken sets the optional parameter "pageToken": The next_page_token value
// to use if there are additional results to retrieve for this list request.
// When paginating, the parent, order_by, and filter parameters must match the
// call that provided the page token (the page_size parameter doesn't need to
// match).
func (c *ProjectsLocationsVmAttachmentsListCall) PageToken(pageToken string) *ProjectsLocationsVmAttachmentsListCall {
	c.urlParams_.Set("pageToken", pageToken)
	return c
}
...
// https://developers.google.com/gdata/docs/2.0/basics#PartialResponse for more
// details.
func (c *ProjectsLocationsVmAttachmentsListCall) Fields(s ...googleapi.Field) *ProjectsLocationsVmAttachmentsListCall {
	c.urlParams_.Set("fields", googleapi.CombineFields(s))
	return c
}
...
// object's ETag matches the given value. This is useful for getting updates
// only after the object has changed since the last request.
func (c *ProjectsLocationsVmAttachmentsListCall) IfNoneMatch(entityTag string) *ProjectsLocationsVmAttachmentsListCall {
	c.ifNoneMatch_ = entityTag
	return c
}

// Context sets the context to be used in this call's Do method.
func (c *ProjectsLocationsVmAttachmentsListCall) Context(ctx context.Context) *ProjectsLocationsVmAttachmentsListCall {
	c.ctx_ = ctx
	return c
}
...
// Header returns a http.Header that can be modified by the caller to add
// headers to the request.
func (c *ProjectsLocationsVmAttachmentsListCall) Header() http.Header {
	if c.header_ == nil {
		c.header_ = make(http.Header)
	}
	return c.header_
...
func (c *ProjectsLocationsVmAttachmentsListCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v1beta1/{+parent}/vmAttachments")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("GET", urls, nil)
	if err != nil {
		return nil, err
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "file.projects.locations.vmAttachments.list", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}

// Do executes the "file.projects.locations.vmAttachments.list" call.
// Any non-2xx status code is an error. Response headers are in either
// *ListVmAttachmentsResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
func (c *ProjectsLocationsVmAttachmentsListCall) Do(opts ...googleapi.CallOption) (*ListVmAttachmentsResponse, error) {
	gensupport.SetOptions(c.urlParams_, opts...)
	res, err := c.doRequest("json")
	if res != nil && res.StatusCode == http.StatusNotModified {
		if res.Body != nil {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "file.projects.locations.vmAttachments.list", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
// A non-nil error returned from f will halt the iteration.
// The provided context supersedes any context provided to the Context method.
func (c *ProjectsLocationsVmAttachmentsListCall) Pages(ctx context.Context, f func(*ListVmAttachmentsResponse) error) error {
	c.ctx_ = ctx
	defer c.PageToken(c.urlParams_.Get("pageToken"))
	for {
		x, err := c.Do()
...
type ProjectsLocationsVmAttachmentsPatchCall struct {
	s            *Service
	nameid       string
...
// Patch: Updates the attachments settings of a specific VM attachment. The
// enactment of the updated VM attachment settings will be done in the
// following order: 1. Detach previous Attachments that are no longer part of
// the updated configuration. 2. Attach newly added Attachments. In the event
// of a failure to detach any of the Attachments from step 1, the
// Attachment.status of all newly added attachments will be set to FAILURE, and
// these will not be actuated until the issue is resolved. For CreateOrUpdate
// behavior, set allow_missing to true.
//
//   - name: Identifier. The resource identifier of this VM attachment, including
//     the VM's project, location, and name (those are the identifiers of the
//     corresponding Compute Engine instance). Format:
//     `projects/{project}/locations/{location}/vmAttachments/{vm_name}`.
func (r *ProjectsLocationsVmAttachmentsService) Patch(nameid string, vmattachment *VmAttachment) *ProjectsLocationsVmAttachmentsPatchCall {
	c := &ProjectsLocationsVmAttachmentsPatchCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.nameid = nameid
	c.vmattachment = vmattachment
	return c
}

// AllowMissing sets the optional parameter "allowMissing": If set to true,
// updating a VmAttachment that does not exist will result in the creation of a
// new VmAttachment. In this situation, `update_mask` is ignored.
func (c *ProjectsLocationsVmAttachmentsPatchCall) AllowMissing(allowMissing bool) *ProjectsLocationsVmAttachmentsPatchCall {
	c.urlParams_.Set("allowMissing", fmt.Sprint(allowMissing))
	return c
}

// UpdateMask sets the optional parameter "updateMask": Mask of fields to
// update in VmAttachment. The only supported field for updating in
// VmAttachment is: `attachments`. If update_mask is empty, then `attachments`
// will be updated only if the provided "vm_attachment.attachments" is not
// empty. To update attachments with an empty configuration, an update_mask
// containing "attachments" must be passed in the request. Note: The path needs
// to be relative to the vm_attachment proto, not the request proto. For
// example, a field mask should be `attachments`, not
// `vm_attachment.attachments`.
func (c *ProjectsLocationsVmAttachmentsPatchCall) UpdateMask(updateMask string) *ProjectsLocationsVmAttachmentsPatchCall {
	c.urlParams_.Set("updateMask", updateMask)
	return c
}
...
// https://developers.google.com/gdata/docs/2.0/basics#PartialResponse for more
// details.
func (c *ProjectsLocationsVmAttachmentsPatchCall) Fields(s ...googleapi.Field) *ProjectsLocationsVmAttachmentsPatchCall {
	c.urlParams_.Set("fields", googleapi.CombineFields(s))
	return c
}

// Context sets the context to be used in this call's Do method.
func (c *ProjectsLocationsVmAttachmentsPatchCall) Context(ctx context.Context) *ProjectsLocationsVmAttachmentsPatchCall {
	c.ctx_ = ctx
	return c
}
...
// Header returns a http.Header that can be modified by the caller to add
// headers to the request.
func (c *ProjectsLocationsVmAttachmentsPatchCall) Header() http.Header {
	if c.header_ == nil {
		c.header_ = make(http.Header)
	}
	return c.header_
...
func (c *ProjectsLocationsVmAttachmentsPatchCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "application/json", c.header_)
	body, err := googleapi.WithoutDataWrapper.JSONBuffer(c.vmattachment)
	if err != nil {
		return nil, err
	}
	c.urlParams_.Set("alt", alt)
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "file.projects.locations.vmAttachments.patch", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}

// Do executes the "file.projects.locations.vmAttachments.patch" call.
// Any non-2xx status code is an error. Response headers are in either
// *Operation.ServerResponse.Header or (if a response was returned at all) in
// error.(*googleapi.Error).Header. Use googleapi.IsNotModified to check
// whether the returned error was because http.StatusNotModified was returned.
func (c *ProjectsLocationsVmAttachmentsPatchCall) Do(opts ...googleapi.CallOption) (*Operation, error) {
	gensupport.SetOptions(c.urlParams_, opts...)
	res, err := c.doRequest("json")
	if res != nil && res.StatusCode == http.StatusNotModified {
		if res.Body != nil {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "file.projects.locations.vmAttachments.patch", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
