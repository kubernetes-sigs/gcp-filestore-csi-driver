...
const (
	paramSharePool                  = "sharepool"
	defaultSharePoolShareCapacityGb = 1
)

func buildSharePoolVolumeID(parentPool, shareID, ipAddress string) string {
	// Scheme format: sharepool://{project}/{location}/{share_pool}/shares/{uuid}?ip={ip_address}
	// parentPool format: projects/{project}/locations/{location}/sharePools/{sharePool}

	hostPath := strings.Replace(parentPool, "projects/", "sharepool://", 1)
...
func isSharePoolVolumeID(volumeID string) bool {
	return strings.HasPrefix(volumeID, "sharepool://")
}
...
func (s *controllerServer) handleCreateSharePoolVolume(ctx context.Context, req *csi.CreateVolumeRequest, sharePoolPath string) (*csi.CreateVolumeResponse, error) {
	...
	if capacityGb == 0 {
		capacityGb = defaultSharePoolShareCapacityGb // Default to 1 GiB if not specified or less than 1 GiB
	}
	...
	shareID := poolShare.ShareName

	compositeVolumeID := buildSharePoolVolumeID(sharePoolPath, shareID, poolShare.IpAddress)

	klog.Infof("Successfully acquired Share Pool volume: %q, Composite ID: %q, IP: %q, ShareName: %q", poolShare.ShareName, compositeVolumeID, poolShare.IpAddress, poolShare.ShareName)
	...
	resp := &csi.CreateVolumeResponse{
		Volume: &csi.Volume{
			...
			VolumeContext: map[string]string{
				...
				attrVolume:       poolShare.ShareName,
				attrFileProtocol: v3FileProtocol, // Standard protocol for GCFS Share Pools is NFS_V3 by default
			},
		},
	}
