...
func initTestMultishareStatefulController(t *testing.T) *MultishareStatefulController {
	...
	msc.shareLister = factory.Multishare().V1().ShareInfos().Lister()
	return msc
}
...
func TestStatefulCreateVolume(t *testing.T) {
			...
			initSi: []*v1.ShareInfo{
				{
					...
					Status: &v1.ShareInfoStatus{
						InstanceHandle: testInstanceName_0,
						CapacityBytes:  100 * util.Gb,
						ShareStatus:    v1.READY,
					},
				},
			},
		},
	...
	for _, tc := range tests {
		...
		for _, si := range tc.initSi {
			msc.clientset.MultishareV1().ShareInfos(util.ManagedFilestoreCSINamespace).Create(context.TODO(), si, metav1.CreateOptions{})
		}
		...
		shareInfoList, err := msc.clientset.MultishareV1().ShareInfos(util.ManagedFilestoreCSINamespace).List(context.TODO(), metav1.ListOptions{})
		if err != nil {
