...
func TestCreateMultishareSnapshot(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "adding tags to multishare snapshot failed",
			req: &csi.CreateSnapshotRequest{
				SourceVolumeId: defaultSourceVolumeID,
				Name:           backupName2,
				Parameters: map[string]string{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			m.tagManager.(*cloud.FakeTagServiceManager).
				On("AttachResourceTags", context.TODO(), cloud.FilestoreBackUp, backupName, testRegion, tc.req.GetParameters()).
				Return(nil)
			m.tagManager.(*cloud.FakeTagServiceManager).
				On("AttachResourceTags", context.TODO(), cloud.FilestoreBackUp, backupName2, testRegion, tc.req.GetParameters()).
				Return(fmt.Errorf("failed to add tags to multishare snapshot"))
