...
func TestCreateVolumeFromSnapshot(t *testing.T) {
	...
	for _, test := range cases {
		...
		cs.config.tagManager.(*cloud.FakeTagServiceManager).
			On("AttachResourceTags", context.TODO(), cloud.FilestoreInstance, testCSIVolume, testLocation, test.req.GetParameters()).
			Return(nil)
...
func TestCreateVolume(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "adding tags to filestore instance failed",
			req: &csi.CreateVolumeRequest{
				Name: testCSIVolume2,
				VolumeCapabilities: []*csi.VolumeCapability{
					{
	...
	for _, test := range cases {
		...
		cs.config.tagManager.(*cloud.FakeTagServiceManager).
			...
			Return(nil)
		cs.config.tagManager.(*cloud.FakeTagServiceManager).
			On("AttachResourceTags", context.TODO(), cloud.FilestoreInstance, testCSIVolume2, testLocation, test.req.GetParameters()).
			Return(fmt.Errorf("fail to add tags"))
...
func TestVolumeOperationLocks(t *testing.T) {
	...
	cs.config.tagManager.(*cloud.FakeTagServiceManager).
		On("AttachResourceTags", context.Background(), cloud.FilestoreInstance, testCSIVolume, testLocation, map[string]string(nil)).
		Return(nil)
	cs.config.tagManager.(*cloud.FakeTagServiceManager).
		On("AttachResourceTags", context.Background(), cloud.FilestoreInstance, testCSIVolume2, testLocation, map[string]string(nil)).
		Return(nil)
...
func TestCreateSnapshot(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "adding tags to filestore backup failed",
			req: &csi.CreateSnapshotRequest{
				SourceVolumeId: "modeInstance/us-central1/myinstance/myshare",
				Name:           backupName2,
				Parameters: map[string]string{
	...
	for _, test := range cases {
		...
		cs.config.tagManager.(*cloud.FakeTagServiceManager).
			On("AttachResourceTags", context.TODO(), cloud.FilestoreBackUp, backupName, region, test.req.GetParameters()).
			Return(nil)
		cs.config.tagManager.(*cloud.FakeTagServiceManager).
			On("AttachResourceTags", context.TODO(), cloud.FilestoreBackUp, backupName, "us-west1", test.req.GetParameters()).
			Return(nil)
		cs.config.tagManager.(*cloud.FakeTagServiceManager).
			On("AttachResourceTags", context.TODO(), cloud.FilestoreBackUp, backupName2, region, test.req.GetParameters()).
			Return(fmt.Errorf("failed to add tags to backup"))
...
func TestDeleteSnapshot(t *testing.T) {
	...
	for _, test := range cases {
		...
		cs.config.tagManager.(*cloud.FakeTagServiceManager).
			On("AttachResourceTags", context.TODO(), cloud.FilestoreBackUp, backupName, region, test.createReq.GetParameters()).
			Return(nil)
