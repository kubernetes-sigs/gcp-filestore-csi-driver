...
func TestControllerExpandVolume(t *testing.T) {
	...
	cases := []testCase{
		...
		{
			name:        "expand volumepool volume is not supported",
			volumeID:    "volumepool://demo-project/us-central1/demo-pool-1/volumes/vol1",
			shouldError: true,
		},
	}
...
func TestCreateSnapshot(t *testing.T) {
	...
	cases := []struct {
		...
		// Failure test cases
		{
			name: "Volume pool volume is not supported",
			req: &csi.CreateSnapshotRequest{
				SourceVolumeId: "volumepool://demo-project/us-central1/demo-pool-1/volumes/vol1",
				Name:           backupName,
			},
			expectErr: true,
		},
...
func TestControllerModifyVolume_VolumePoolNotSupported(t *testing.T) {
	ctrl := initTestController(t)
	req := &csi.ControllerModifyVolumeRequest{
		VolumeId: "volumepool://demo-project/us-central1/demo-pool-1/volumes/vol1",
	}
	...
	if err == nil {
		t.Fatalf("expected error for volumepool volume id, got nil")
	}
	st, ok := status.FromError(err)
	if !ok || st.Code() != codes.InvalidArgument {
