...
func (f *FakeTagServiceManager) ValidateResourceTags(ctx context.Context, tags string) (resourceTags, error) {
...
func (f *FakeTagServiceManager) AttachResourceTags(ctx context.Context, rscType resourceType, rscName, rscLocation string, reqParameters map[string]string) error {
	rets := f.Called(ctx, rscType, rscName, rscLocation, reqParameters)
	e, ok := rets[0].(error)
	if !ok {
		return nil
	}
