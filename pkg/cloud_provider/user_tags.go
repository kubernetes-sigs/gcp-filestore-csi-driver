...
type TagService interface {
	...
	ValidateResourceTags(context.Context, string) (resourceTags, error)
	AttachResourceTags(context.Context, resourceType, string, string, map[string]string) error
}
...
// tagValuesClient handles operations on tag binding resources.
...
func extractTags(ctx context.Context, tagService TagService, parameters map[string]string) (resourceTags, error) {
	if parameters != nil {
		if tags, exist := parameters[ParameterKeyResourceTags]; exist {
			return tagService.ValidateResourceTags(ctx, tags)
		}
	}
	return nil, nil
}
...
func (t *tagServiceManager) ValidateResourceTags(ctx context.Context, commaSeparatedTags string) (resourceTags, error) {
	const (
		tagListDelimiter   = ","
	...
	klog.V(5).Infof("configured list of resource tags: %s", commaSeparatedTags)
	tagList := strings.Split(commaSeparatedTags, tagListDelimiter)

	if len(tagList) > maxTagsPerResource {
		return nil, fmt.Errorf("more than %d tags is not allowed, given: %d", maxTagsPerResource, len(tagList))
	}
	...
	for _, tag := range tagList {
		...
		if c := strings.Count(name, tagsDelimiter); c != tagsDelimiterCount {
			return nil, fmt.Errorf("%s tag not in expected format(<parentID/tagKey_name/tagValue_name>)", name)
		}
		if err := client.validateTagExist(ctx, name); err != nil {
			...
			if errors.As(err, &gErr) && gErr.HTTPCode() == http.StatusForbidden {
				klog.V(5).Infof("does not have permission to access %s tag or does not exist", name)
	...
	if len(nonexistentTags) != 0 {
		return nil, fmt.Errorf("%v tag(s) does not exist", nonexistentTags)
	}

	(&tags).removeDuplicateTags()

	klog.V(1).Infof("validated list of resource tags: %v", tags)
	return tags, nil
}
...
func (t *tagServiceManager) AttachResourceTags(ctx context.Context, rscType resourceType, rscName, rscLocation string, reqParameters map[string]string) error {
	tags, err := extractTags(ctx, t, reqParameters)
	if err != nil {
		return err
	}
	...
	klog.V(1).Infof("list of tags attached to %s: %v", rscName, effectiveTags)
...
func (c *tagValuesClient) close() {
	if err := c.Close(); err != nil {
		klog.V(1).ErrorS(err, "failed to close tag value client connection")
	}
}
...
func (c *tagBindingsClient) close() {
	if err := c.Close(); err != nil {
		klog.V(1).ErrorS(err, "failed to close tag binding client connection")
	}
}
...
func (c *tagBindingsClient) getTagsToBind(ctx context.Context, rscName string, tags resourceTags) (resourceTags, error) {
	...
	klog.V(1).Infof("refined list of resource tags: %v", refinedTags)
