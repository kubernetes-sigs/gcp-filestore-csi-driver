...
func TestRemoveDuplicateTags(t *testing.T) {
	cases := []struct {
		...
		{
			name: "invalid tags does not panic",
			tags: resourceTags{
				"openshift/test1/test1":  resourceTagsValueFiller,
				"openshift/test2/test2":  resourceTagsValueFiller,
				"openshift/test1/test11": resourceTagsValueFiller,
		...
		{
			name: "invalid tags panic expected",
			tags: resourceTags{
				"openshift/test1/test1":  resourceTagsValueFiller,
				"openshift/test2/test2":  resourceTagsValueFiller,
				"openshift/test1/test11": resourceTagsValueFiller,
...
func TestExtractTags(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			tags, err := extractTags(ctx, tagMgr, test.parameters)
...
func TestValidateResourceTags(t *testing.T) {
	cases := []struct {
		...
		{
			name:               "gapi internal error",
			commaSeparatedTags: "openshift/test1/test1,openshift/test4/test4",
			expectedTags:       resourceTags{},
			expectedErr:        "[openshift/test4/test4] tag(s) does not exist",
		},
		{
			...
			expectedTags: resourceTags{},
			expectedErr:  "more than 50 tags is not allowed, given: 51",
		},
		{
			name:               "invalid tag configured",
			commaSeparatedTags: "openshift/test1/test1,test4/test4",
			expectedTags:       resourceTags{},
			expectedErr:        "test4/test4 tag not in expected format(<parentID/tagKey_name/tagValue_name>)",
		},
	}
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			tags, err := tagMgr.ValidateResourceTags(ctx, test.commaSeparatedTags)
...
func TestAttachResourceTags(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:        "empty user defined tags, invalid parameters tags",
			rscType:     FilestoreInstance,
			rscName:     filestoreInstanceName,
			rscLocation: rscLocation,
			reqParameters: map[string]string{
		...
		{
			name:             "create tag bindings fails",
			rscType:          FilestoreInstance,
			rscName:          filestoreInstanceName,
			rscLocation:      rscLocation,
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			err := tagMgr.AttachResourceTags(ctx, test.rscType, test.rscName, test.rscLocation, test.reqParameters)
			if (err != nil || test.expectedErr != "") && err.Error() != test.expectedErr {
...
func TestGetTagsToBind(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:    "listing effective tags fails",
			rscName: rscName2,
			tags: resourceTags{
				"openshift/test1/test1": resourceTagsValueFiller,
				"openshift/test2/test2": resourceTagsValueFiller,
...
func TestCreateTagBindings(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:        "tag bindings creation wait failure",
			rscName:     rscName,
			tags:        tags2,
		...
		{
			name:        "tag bindings creation fails",
			rscName:     rscName,
			tags:        tags4,
			expectedErr: fmt.Sprintf("failed to add tag(s) to %s resource", rscName),
		},
