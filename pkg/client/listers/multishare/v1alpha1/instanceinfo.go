...
type InstanceInfoLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.InstanceInfo, err error)
	// Get retrieves the InstanceInfo from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.InstanceInfo, error)
...
// List lists all InstanceInfos in the indexer.
func (s *instanceInfoLister) List(selector labels.Selector) (ret []*v1alpha1.InstanceInfo, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.InstanceInfo))
	})
	return ret, err
}

// Get retrieves the InstanceInfo from the index for a given name.
func (s *instanceInfoLister) Get(name string) (*v1alpha1.InstanceInfo, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("instanceinfo"), name)
	}
	return obj.(*v1alpha1.InstanceInfo), nil
}
