...
type InstanceInfoLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.InstanceInfo, err error)
...
// List lists all InstanceInfos in the indexer.
func (s *instanceInfoLister) List(selector labels.Selector) (ret []*v1beta1.InstanceInfo, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.InstanceInfo))
	})
	return ret, err
}
...
type InstanceInfoNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.InstanceInfo, error)
...
// List lists all InstanceInfos in the indexer for a given namespace.
func (s instanceInfoNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.InstanceInfo, err error) {
...
// Get retrieves the InstanceInfo from the indexer for a given namespace and name.
func (s instanceInfoNamespaceLister) Get(name string) (*v1beta1.InstanceInfo, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("instanceinfo"), name)
	}
	return obj.(*v1beta1.InstanceInfo), nil
}
